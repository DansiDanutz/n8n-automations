...
require(limiter_env, "TRUSTED_PROXY_CIDRS=", "limiter template must require explicit trusted proxy networks")
require(limiter_docker, "--no-proxy-headers", "rate limiter must disable Uvicorn forwarding-header rewrites")
require(limiter, "proxy_headers=False", "rate limiter direct startup must disable Uvicorn forwarding-header rewrites")
...
require(smart_lead, '"authentication": "headerAuth"', "smart lead webhook must authenticate callers")
require(smart_lead, '"name": "Validate Lead Input"', "smart lead workflow must validate untrusted lead fields")
require(smart_lead, '"name": "Validate AI Score"', "smart lead workflow must constrain model output before routing")
